...
@implementation PayButtonViewController
...
- (void)loadView {
   /* 
    UIActionSheet *asheet = [[UIActionSheet alloc] initWithTitle:@"Select an amount" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"$10", @"$20", @"$50",@"$100",@"Custom", nil];
    
    [asheet showFromTabBar:appDelegate.tabBarController.tabBar];
    [asheet release]; */
    
    NSLog (@"%@", @"Loaded PayButtonViewController!");
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (buttonIndex != actionSheet.cancelButtonIndex && buttonIndex != 4) {
		...
		if (buttonIndex == 0) {
			thisAmount = [[NSDecimalNumber alloc] initWithUnsignedInteger:10];
            NSLog(@"Amount is: %@",thisAmount);
		} else if (buttonIndex == 1) {
			thisAmount = [[[NSDecimalNumber alloc] initWithUnsignedInteger:20]retain];
		} else if (buttonIndex == 2) {
            thisAmount = [[[NSDecimalNumber alloc] initWithUnsignedInteger:50]retain];
		} else if (buttonIndex == 3) {
			thisAmount = [[[NSDecimalNumber alloc] initWithUnsignedInteger:100]retain];
		}	 
...
- (void)simplePayment {
    //exp
 //   NSLog(@"myAmount is: %@",[myprof thisAmount] );
	...
	payment.recipient = @"seedsmicroloans@gmail.com";
	payment.paymentCurrency = @"USD";
	payment.description = @"Add Credit to Account";
	payment.merchantName = @"seedsmicroloans@gmail.com";
	
	//subtotal of all items, without tax and shipping
    
   // ProfileViewController *amount = [[[ProfileViewController alloc] init] autorelease];
    
  //  NSLog(@"credo is: %@", amount.thisAmount);
    
//	payment.subTotal = [NSDecimalNumber decimalNumberWithString:@"10.00"];
    
    payment.subTotal = appDelegate.myCredit;
    NSLog(@"subTotal: %@", payment.subTotal);
...
- (void)preapproval {
	...
	[[PayPal getPayPalInst] preapprovalWithKey:preapprovalField.text andMerchantName:@"seedsmicroloans@gmail.com"];
}
...
- (void)paymentSuccessWithKey:(NSString *)payKey andStatus:(PayPalPaymentStatus)paymentStatus {
  ...
  //  NSDecimalNumber *thisAmount = [[NSDecimalNumber alloc]initWithString:@"10.0"];
    //new addition
//    appDelegate = [[UIApplication sharedApplication] delegate];

    [appDelegate.currentLender addCredit:appDelegate.myCredit];
    /*
//    NSLog (@"%@", @"Going to start new thread");
//    [NSThread detachNewThreadSelector:@selector(recordPayKey) toTarget:self withObject:nil]; 
    
 //   [self.payDelegate transactionsuccessful:payKey];
 //   [appDelegate.currentLender addCredit:10];
 //   NSLog (@"%@", @"Just called delegate");
 
    //BOOK KEEPING....
    NSLog (@"%@", @"Going to do book keeping");
    
//    appDelegate = [[UIApplication sharedApplication] delegate];
    
  //  NSLog(@"%@",appDelegate.currentLender.uid);
    NSString *myid, *myamount;
    myid = [[NSString alloc] initWithString:@"12345"];
    myamount = [[NSString alloc] initWithString:@"10"];

  //  NSLog(@"The value of myid is %i", myid);
    
    NSDictionary *putDictionary = [NSDictionary dictionaryWithObjectsAndKeys:myid,@"uid",payKey,@"payKey",myamount,@"amount",nil];
...
//    RecordTransactions *newSender = [[RecordTransactions alloc] initWithParams:@"paypaltransactions" apiName:@"record" jsonString:jsondict apiCall:@"CREATE"];
//    [newSender release]; 

    //Send badge via POST request
    
    Sender *newBadgeSender = [[Sender alloc] initWithParams:@"paypaltransactions" jsonString:jsondict idToBeChanged:@"" apiCall:@"POST"];
